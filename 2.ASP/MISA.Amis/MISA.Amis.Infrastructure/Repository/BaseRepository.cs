...
namespace MISA.Amis.Infrastructure.Repository
{
    public class BaseRepository<MISAEntity> : IBaseRepository<MISAEntity>
    {
        ...
        //:                                       ^ id của nv/kh
        {
            ...
            parameters.Add($"m_{_entityName.ToLower()}Id", entityId);

            // Lấy dữ liệu và phản hồi cho client

            _dbConnection.Open();

            var res = _dbConnection.QueryFirstOrDefault<MISAEntity>(procName, param: parameters, commandType: CommandType.StoredProcedure);
